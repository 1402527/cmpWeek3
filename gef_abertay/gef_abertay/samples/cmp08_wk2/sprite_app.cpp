...
bool SpriteApp::Update(float frame_time)
{
	...
	// get the latest data form from the input devices
	if (input_manager_) 
	{
		...
		if (controller_input) 
		{
			// read the controller data for controller 0
			const gef::SonyController* controller = controller_input->GetController(0);

			if (controller) 
			{
				// tests for X button being held down
				...
				// tests for X button being pressed
				if (controller->buttons_pressed() & gef_SONY_CTRL_CROSS)
				{
					// do something
					gef::DebugOut("x has been pressed\n");
				}

				// tests for X button being released
				if (controller->buttons_released() & gef_SONY_CTRL_CROSS)
				{
					// do something
					gef::DebugOut("x has been released\n");
				}

				// tests for start and select button being held down
				if ((controller->buttons_down() & (gef_SONY_CTRL_START | gef_SONY_CTRL_SELECT)) == (gef_SONY_CTRL_START | gef_SONY_CTRL_SELECT))
				{
					// do something
					gef::DebugOut("start and select are being held down\n");
					return 0;
				}
				...
				gef::DebugOut("lx: %f ", left_x_axis);
				gef::DebugOut("ly: %f ", left_y_axis);
				gef::DebugOut(" rx: %f ", right_x_axis);
				gef::DebugOut(" ry: %f\n", right_y_axis);
			}
		}
	}
